...
$query = "Select count(*) as br from proizvod where ID_Proizvod in(SELECT id_proizvod as br FROM listazelja)";
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
	  ...
	  <li> 
        ...
        <li class="active"><a href="zelje.php"><span class="glyphicon glyphicon-shopping-cart"></span>Kosarice</a></li>
...
$query = "SELECT ID_Proizvod,Naziv,Cijena FROM `proizvod` where ID_Proizvod in(SELECT id_proizvod from listazelja)";	
$result = db_select($query);
...
<br><div class="middle'.$pocetak.' ">
			');
			if($_SESSION['ovlasti']==1){
				$query1 = "SELECT * FROM `registrirani` where ID_REG in(SELECT id_kupca from listazelja where ID_Proizvod='".$result[$pocetak]["ID_Proizvod"]."')";	
				$kupci = db_select($query1); 
				if(isset($kupci[0]['Mail'])){
print('<a href="Profil.php?id='.$kupci[0]['ID_REG'].'" class="text">'.$kupci[0]['Mail'].'</a>');
				}
				print('<a  href="UrediZapis.php?id='.$result[$pocetak]["ID_Proizvod"].'" class="text">Nadopuni</a>');
			}else{
